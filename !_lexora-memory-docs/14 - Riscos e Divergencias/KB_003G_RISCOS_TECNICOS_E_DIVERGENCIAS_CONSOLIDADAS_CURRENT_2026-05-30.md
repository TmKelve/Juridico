...
| RISK-008 | P1 | Dados | storageAdapter de documentos — disco local efêmero no Render | `createLocalDocumentStorageAdapter()` em `main.ts:667` usa `fs.writeFile` em `./storage/documents/` — disco efêmero | Documentos perdidos a cada restart/redeploy | ✅ **resolvido** (2026-05-30) | BL-045, BL-058 | `createCloudDocumentStorageAdapter()` via `@vercel/blob` implementado. Seleção automática por `BLOB_READ_WRITE_TOKEN`. Pendente: Tom criar Blob Store no Vercel + configurar token no Render. Commit `4bb3db1`. |
...
| RISK-012 | P1 | Segurança | Sem rate limiting em `/ai/*` | `register-ai-routes.ts` sem middleware de rate limit | Custo ilimitado se LLM externo ativado | ✅ **resolvido** (2026-05-30) | BL-074 | `app.use('/ai', aiRateLimit)` — 10 req/min via `express-rate-limit`. Commit `94a8c91`. |
...
| RISK-015 | P1 | Segurança | Dev mock com credenciais em plaintext no backend | `main.ts` com usuários e senhas hardcoded | Exposição de credenciais de dev em clones do repo | ✅ **parcialmente resolvido** (2026-05-30) | BL-042 | Senhas movidas para `LEXORA_DEV_PASSWORD` e `LEXORA_SEED_PASSWORD` (env vars). Fallback `'123456'` permanece no código como default de dev — risco aceito. Commit `94a8c91`. |
| RISK-016 | P1 | Segurança | Sem rate limiting em `/auth/login` | Nenhum middleware identificado no KB-003C | Vulnerável a brute force | ✅ **resolvido** (2026-05-30) | BL-043 | `loginRateLimit`: 20 req/15min via `express-rate-limit`. `trust proxy=1` configurado para IP correto atrás do Render. Commit `94a8c91`. |
| RISK-017 | P1 | Segurança | Sem headers de segurança HTTP (Helmet) | Nenhum middleware de headers identificado | HSTS, X-Content-Type e outros ausentes | ✅ **resolvido** (2026-05-30) | BL-044 | `helmet()` aplicado globalmente antes do CORS. CSP e COEP desabilitados (frontend no Vercel gerencia). Commit `94a8c91`. |
...
| RISK-022 | P2 | Backend | Role check em `/admin/seed-finance` incorreto | `actor.role !== 'admin'` mas roles reais são `ADM`, `ADV` | Endpoint pode ser acessível indevidamente | ✅ **resolvido** (2026-05-30) | BL-041 | Corrigido para `actor.role !== 'ADM'`. Commit `fae145a`. |
